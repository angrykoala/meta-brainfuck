...
void bf_file_parser::save(const brainfuck_interpreter &interpreter) const {
    ...
    file<<"{"<<memory_pointer<<"}";
    list<unsigned char> memory=interpreter.get_memory();
    file<<"{ ";
    for(unsigned char l:memory) {
        file<<to_string(l)<<" ";
    }
    file<<"}"<<endl;
...
brainfuck_interpreter bf_file_parser::parse_content(const string &content) const {
    ...
    list<unsigned char> memory = this->parse_memory(m[3]);
    return brainfuck_interpreter(m[4], code_position, memory, memory_pointer);
}

list<unsigned char> bf_file_parser::parse_memory(const string &raw_memory) const {
    list<unsigned char> memory;
    regex r(R"(\d+)");
    sregex_iterator iter(raw_memory.begin(), raw_memory.end(), r);
    sregex_iterator end;
    while(iter != end) {
        unsigned char value = stoi((*iter)[0]);
